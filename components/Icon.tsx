...
const PATHS: Record<string, string> = {
  ...
  send:           "M22 2L11 13M22 2L15 22 11 13 2 9Z",
  // ── Layout ──────────────────────────────────────────────────
  ...
  // 15/09/2026 — módulo Ótica (duas lentes, ponte e hastes). Estilo Lucide.
  glasses:        "M6 18a4 4 0 100-8 4 4 0 000 8z M18 18a4 4 0 100-8 4 4 0 000 8z M10 14h4 M2 14l1.5-6A2 2 0 015.4 6.5H6 M22 14l-1.5-6a2 2 0 00-1.9-1.5H18",
...
// Icones que se desenham PREENCHIDOS, nao contornados.
//
// O resto do set e estilo Lucide: traco de 2px, fill="none". Marcas de
// terceiros nao seguem essa gramatica -- o logo do WhatsApp e uma silhueta
// solida, e passar ela pelo pipeline de stroke desenha o contorno da
// silhueta (a 15px, um borrao).
//
// Alem do fill, estes NAO podem ser quebrados em varios <path>: o recorte do
// fone dentro do balao depende de os subpaths ficarem no MESMO path, com
// winding oposto (fill-rule nonzero). O split(" M") usado abaixo destruiria
// o recorte e devolveria um balao chapado.
const FILLED = new Set<string>(["whatsapp"]);
...
export function Icon({ name, size = 20, color = "#a0a0b8" }: IconProps) {
  ...
  if (Platform.OS === "web") {
    const svg = FILLED.has(resolved)
      ? `<svg width="${size}" height="${size}" viewBox="0 0 24 24" fill="${color}"><path d="${path}"/></svg>`
      : `<svg width="${size}" height="${size}" viewBox="0 0 24 24" fill="none" stroke="${color}" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">${path
          .split(" M")
