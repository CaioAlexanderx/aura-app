...
// Opacidade única da marca d'água (frente = verso), por pedido da federação.
...
function renderFront(card: MembershipCard, options?: CarteirinhaBatchOptions): string {
  ...
  // Nº CBKT da faixa vigente (só faixa-preta, quando houver). Fica ao lado
  // do campo Faixa, na última linha do corpo.
  const cbkt = esc(card.cbkt_number || "");
  const beltField =
  ...
  const beltRow = cbkt
    ? '<div class="grid2 reg-fld">' + beltField +
        '<div class="fld"><div class="flabel">Nº CBKT</div><div class="fvalue mono cbkt-num">' + cbkt + '</div></div>' +
      '</div>'
    : '<div class="reg-fld">' + beltField + '</div>';

  const fieldsGrid = isPreta
    ? (
      // Design 02: grid 1fr 1fr alinhado (igual ao mock aprovado) + Faixa/CBKT no fim.
      '<div class="grid2">' +
...
// CSS compartilhado do cartão (reset + regras .cr80…). Usado TANTO pela
// impressão em lote (buildCarteirinhaHtml) QUANTO pelo preview de cartão único
// (buildSingleCardHtml) — garante que a visualização na aba Praticantes fique
// idêntica à impressão (mesma fonte-verdade: este arquivo).
function cardCss(): string {
  return [
    '*{margin:0;padding:0;box-sizing:border-box}',
    '.cr80{width:' + CARD_W_MM + 'mm;height:' + CARD_H_MM + 'mm;background:#ffffff;border-radius:2.6mm;border:0.18mm solid ' + LINE_2 + ';overflow:hidden;page-break-inside:avoid;break-inside:avoid;position:relative}',
    '.face-pad{position:relative;z-index:2;height:100%;display:flex;flex-direction:column;padding:4.2mm 5.1mm 3.5mm}',
    '.wm{position:absolute;pointer-events:none;object-fit:contain;z-index:1}',
    // Opacidade da marca d'água IGUAL na frente e no verso (pedido federação).
    '.wm-front{left:70%;top:50%;width:52mm;opacity:' + WM_OPACITY + ';transform:translate(-50%,-50%)}',
    '.wm-back{left:50%;top:56%;width:41mm;opacity:' + WM_OPACITY + ';transform:translate(-50%,-50%)}',
    '.head{display:flex;align-items:flex-start;justify-content:space-between;min-height:7.3mm}',
    '.head-left{display:flex;align-items:center;gap:2.1mm;flex:1;min-width:0}',
    '.logo{flex-shrink:0;display:flex;align-items:center}',
    '.logo-img{width:10.2mm;height:auto;object-fit:contain}',
    '.logo-fallback{font-size:4mm;color:' + RED + '}',
    '.fed-name{font-family:"Shippori Mincho",serif;font-size:7.4pt;font-weight:700;letter-spacing:0.05pt;color:' + INK + ';line-height:1.26}',
    '.fed-name>div{white-space:nowrap;overflow:hidden;text-overflow:ellipsis}',
    '.head-right{flex-shrink:0;text-align:right}',
    '.hd-carteira{font-family:"Shippori Mincho",serif;font-size:7.3pt;font-weight:400;color:' + INK_2 + ';text-align:right}',
    '.hd-sub{font-size:3.7pt;letter-spacing:0.6pt;text-transform:uppercase;color:' + INK_3 + ';text-align:right;margin-top:0.6mm}',
    '.hd-badge{font-size:5pt;letter-spacing:0.5pt;text-transform:uppercase;color:' + INK_2 + ';font-weight:500;display:flex;align-items:center;gap:0.8mm;justify-content:flex-end;margin-top:0.7mm}',
    '.badge-sq{width:1.3mm;height:1.3mm;background:' + BLACK_BAR + ';display:inline-block}',
    '.hd-verso{font-size:3.7pt;letter-spacing:0.7pt;text-transform:uppercase;color:' + INK_3 + '}',
    '.ruler-red{margin:2.4mm -5.1mm 0;height:0.3mm;background:' + RED + '}',
    '.black-bar{margin:0.8mm -5.1mm 0;height:1.05mm;background:' + BLACK_BAR + '}',
    '.body-row{display:flex;gap:4.3mm;margin-top:2.4mm;flex:1;min-height:0;align-items:flex-start}',
    '.photo{width:21.7mm;height:28.9mm;flex-shrink:0;border-radius:1.4mm;border:0.15mm solid ' + LINE_2 + ';object-fit:cover;background:#faf8f3}',
    '.photo-empty{display:flex;flex-direction:column;align-items:center;justify-content:center;gap:0.4mm;font-family:"DM Mono",monospace;font-size:4.6pt;letter-spacing:0.5pt;color:' + INK_4 + '}',
    '.photo-sub{font-size:4.2pt}',
    '.fields{flex:1;min-width:0;display:flex;flex-direction:column;gap:2.4mm}',
    '.flabel{font-family:"DM Mono",monospace;font-size:3.9pt;letter-spacing:0.5pt;text-transform:uppercase;color:' + INK_3 + '}',
    '.fvalue{font-family:"Zen Kaku Gothic New",sans-serif;font-size:6.4pt;font-weight:500;color:' + INK + ';margin-top:0.7mm;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}',
    // Dojô: nome comprido em até 3 linhas com entrelinha justa e ALTURA
    // RESERVADA fixa — a linha CPF/Nº registro abaixo não desloca.
    '.fvalue.dojo{white-space:normal;display:-webkit-box;-webkit-line-clamp:3;-webkit-box-orient:vertical;line-height:1.0;font-size:5.8pt;height:calc(5.8pt * 3);overflow:hidden}',
    '.fvalue.name{font-size:9pt;font-weight:700;white-space:normal}',
    '.fvalue.mono{font-family:"DM Mono",monospace;font-weight:400}',
    '.name-fld{margin-bottom:0.4mm}',
    '.grid2{display:grid;grid-template-columns:1fr 1fr;gap:2.6mm 2.7mm}',
    '.reg-fld{margin-top:1.6mm}',
    '.is-preta .body-row{margin-top:1.7mm}',
    '.is-preta .fields{gap:1.9mm}',
    '.is-preta .name-fld{margin-bottom:0.2mm}',
    '.is-preta .reg-fld{margin-top:1.0mm}',
    '.reg-num{font-size:7.6pt;font-weight:500;color:' + RED + ';letter-spacing:0.2pt}',
    '.cbkt-num{font-size:7pt;font-weight:500;color:' + INK + ';letter-spacing:0.2pt}',
    '.belt-line{display:flex;align-items:center;gap:1.1mm;margin-top:0.7mm}',
    '.belt-sq{width:1.9mm;height:1.9mm;background:' + BLACK_BAR + ';border-radius:0.3mm;flex-shrink:0}',
    '.belt-label{margin-top:0;font-weight:700}',
    '.back-row{display:flex;flex:1;margin-top:1.8mm;min-height:0}',
    '.kun-col{flex:1.45;padding-right:3.2mm;display:flex;flex-direction:column;justify-content:flex-start}',
    '.kun-eyebrow{font-family:"DM Mono",monospace;font-size:3.9pt;letter-spacing:0.55pt;text-transform:uppercase;color:' + RED + '}',
    '.kun-title{font-family:"Shippori Mincho",serif;font-size:6.4pt;font-weight:500;margin-top:0.4mm;color:' + INK + '}',
    '.kun-list{margin-top:1.2mm;display:flex;flex-direction:column;gap:0.95mm}',
    '.kun-item{display:flex;align-items:flex-start;gap:1.5mm}',
    '.kun-dot{width:0.8mm;height:0.8mm;background:' + RED + ';margin-top:0.7mm;flex-shrink:0}',
    '.kun-text{font-size:5pt;line-height:1.2;color:' + INK + '}',
    '.verify-col{flex:1;border-left:0.15mm solid ' + LINE + ';padding-left:3.2mm;display:flex;flex-direction:column;align-items:center;justify-content:flex-start;text-align:center}',
    '.verify-eyebrow{font-family:"DM Mono",monospace;font-size:3.9pt;letter-spacing:0.55pt;text-transform:uppercase;color:' + INK_3 + '}',
    '.verify-title{font-family:"Shippori Mincho",serif;font-size:6.8pt;font-weight:500;margin-top:0.9mm;color:' + INK + '}',
    '.qr{width:14.2mm;height:14.2mm;margin-top:1.6mm;background:#fff;image-rendering:pixelated}',
    '.verify-num{font-family:"DM Mono",monospace;font-size:5.2pt;color:' + INK + ';margin-top:1.2mm;letter-spacing:0.2pt}',
    '.issued-col{margin-top:1.5mm}',
    '.issued-label{font-family:"DM Mono",monospace;font-size:3.4pt;letter-spacing:0.5pt;text-transform:uppercase;color:' + INK_3 + '}',
    '.issued-value{font-family:"DM Mono",monospace;font-size:5.2pt;margin-top:0.5mm;color:' + INK + '}',
  ].join('');
}

const FONTS_IMPORT = '@import url(\'https://fonts.googleapis.com/css2?family=Shippori+Mincho:wght@400;500&family=Zen+Kaku+Gothic+New:wght@400;500;700&family=DM+Mono:wght@400;500&display=swap\');';

// Preview de cartão ÚNICO (uma face), sem grid/topbar/botão de imprimir.
// Renderizado num <iframe> na aba Praticantes para o preview ficar 1:1 com a
// impressão. bg transparente; o cartão é centralizado e escala via CSS no host.
export function buildSingleCardHtml(card: MembershipCard, face: "front" | "back", options?: CarteirinhaBatchOptions): string {
  const faceHtml = face === "back" ? renderBack(card, options) : renderFront(card, options);
  let html = '<!doctype html><html lang="pt-BR"><head><meta charset="UTF-8">';
  html += '<meta name="viewport" content="width=device-width, initial-scale=1">';
  html += '<style>' + FONTS_IMPORT;
  html += 'html,body{margin:0;padding:0;background:transparent;overflow:hidden}';
  html += 'body{font-family:"Zen Kaku Gothic New",system-ui,sans-serif;color:' + INK + '}';
  html += cardCss();
  // O cartão é renderizado no tamanho físico CR80 (mm/pt) e escalado por
  // transform para preencher a largura do iframe — preservando TODAS as
  // proporções (mm/pt escalam junto), ao contrário de width:100% que quebraria.
  html += '.scaler{transform-origin:top left}';
  html += '</style></head><body>';
  html += '<div class="scaler">' + faceHtml + '</div>';
  html += '<script>(function(){' +
    'var el=document.querySelector(".scaler");' +
    'function fit(){el.style.transform="none";var w=el.offsetWidth;var avail=document.documentElement.clientWidth||w;var s=avail/w;el.style.transform="scale("+s+")";document.body.style.height=(el.offsetHeight*s)+"px";}' +
    'if(document.fonts&&document.fonts.ready){document.fonts.ready.then(fit);}fit();window.addEventListener("resize",fit);setTimeout(fit,300);' +
    '})();</script>';
  html += '</body></html>';
  return html;
}
...
export function buildCarteirinhaHtml(cards: MembershipCard[], options?: CarteirinhaBatchOptions): string {
  ...
  // CSS do cartão compartilhado com o preview (buildSingleCardHtml) — fonte-verdade única.
  html += cardCss();
