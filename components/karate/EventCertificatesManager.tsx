...
// Modelo de partida (usa as mesmas tags do texto padrão) — evita começar do zero.
...
export function EventCertificatesManager({ federationId, event, federationName }: {
  ...
  // Insere a etiqueta {tag} no fim do texto (com espaço se necessário).
  const insertTag = (tag: string) =>
    setBodyText((t) => (t && !/\s$/.test(t) ? t + " " : t) + `{${tag}}`);
  // Ao ligar o texto personalizado com o campo vazio, começa do modelo padrão
  // (evita a tela em branco confusa) — desligar não apaga o que foi escrito.
