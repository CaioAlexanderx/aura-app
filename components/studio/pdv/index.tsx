...
export default function StudioCaixaPage() {
  ...
  const handleQuickAdd = useCallback(
    (p: StudioProduct, rect?: FlyRect | null) => {
      ...
      if (!reducedMotion) {
        flyToCart(rect, (p.name || "?").charAt(0).toUpperCase(), t.primary);
      }
    },
  ...
  // carrega templates ao abrir o configurador
  useEffect(() => {
  ...
  function confirmConfigure(
    ...
    toast.success(
      editLineId
        ...
        : configure.product.name + " personalizado e adicionado",
    );
    setConfigure(null);
  }
        ...
        >
          {catalog.categories.map((c) => (
            <CategoryChip
              ...
              label={c.label}
              count={c.count}
              active={catalog.cat === c.id}
              onPress={() => catalog.setCat(c.id)}
            />
          ))}
        </ScrollView>
  ...
  const MainScroll = (
    <ScrollView
      ...
      contentContainerStyle={{
        paddingBottom: wide ? 0 : cart.count > 0 ? 96 : 24,
      }}
    >
      <Hero
        t={t}
    ...
    <View style={{ flex: 1, backgroundColor: t.bg }}>
      ...
      {configure && (
        <StageConfigure
          ...
          initialValues={configure.values}
          initialQty={configure.qty}
          editLineId={configure.editLineId}
          templates={catalog.templatesById[configure.product.id] || []}
