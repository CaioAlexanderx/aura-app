...
export function StoreNav({ menu, ativa, onSelect, primary }: Props) {
  ...
  return (
    <View
      ...
      // aberto quando o ponteiro passa direto para a lista.
      onMouseLeave={Platform.OS === "web" ? (fechar as any) : undefined}
    >
      ...
      {abertoItem && abertoItem.filhas.length > 0 && (
        <Painel itens={abertoItem.filhas} titulo={abertoItem.name} />
      )}
      {mostrandoExtras && <Painel itens={menu.extras} titulo="Todas as categorias" />}
    </View>
  );
}
