...
// Chama backend POST /dental/ai/consulta de verdade.
// 4 estados: plano abaixo / sem opt-in / sem consent / OK.
// Apenas intent=qa aqui. Brief auto mora em ConsultaIntro.
// Summarize mora no ConsultaEndModal.
// ============================================================
...
export function ConsultaAiPanel({ appointmentId, patientId, briefSeed }: Props) {
  ...
  useEffect(() => {
    ...
    setMessages([{ id: uid(), role: "ai", text: briefSeed, meta: { intent: "brief" } }]);
  }, [briefSeed]);
    ...
    <View style={{ flex: 1, padding: 12, backgroundColor: DentalColors.bg }}>
      ...
      <View style={{ flexDirection: "row", gap: 6, marginTop: 6 }}>
        <TextInput value={input} onChangeText={setInput}
          ...
          editable={!askMut.isPending} onSubmitEditing={send} returnKeyType="send"
          style={{ flex: 1, padding: 8, fontSize: 11, color: DentalColors.ink,
            backgroundColor: DentalColors.surface, borderWidth: 1, borderColor: DentalColors.border, borderRadius: 6 }} />
        <Pressable onPress={send} disabled={!input.trim() || askMut.isPending}
...
function GatedView(props: {
  ...
  return (
    <View style={{ flex: 1, padding: 12, backgroundColor: DentalColors.bg }}>
      <Text style={{ fontSize: 9, color: DentalColors.ink3, fontWeight: "700",
        letterSpacing: 1.4, textTransform: "uppercase", marginBottom: 8 }}>✨ IA AURA</Text>
      <View style={{ flex: 1, backgroundColor: DentalColors.bg2,
        borderWidth: 1, borderColor: "rgba(124,58,237,0.30)",
        borderRadius: 10, padding: 16,
        alignItems: "center", justifyContent: "center", gap: 10 }}>
        <Text style={{ fontSize: 28 }}>{props.icon}</Text>
        <Text style={{ fontSize: 13, color: DentalColors.ink, fontWeight: "700", textAlign: "center" }}>{props.title}</Text>
        <Text style={{ fontSize: 11, color: DentalColors.ink2, textAlign: "center", lineHeight: 16, maxWidth: 320 }}>{props.desc}</Text>
        ...
        {props.footer ? <Text style={{ fontSize: 9, color: DentalColors.ink3, marginTop: 4 }}>{props.footer}</Text> : null}
      </View>
    </View>
  );
}
