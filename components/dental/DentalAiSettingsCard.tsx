...
export function DentalAiSettingsCard() {
  ...
  function handleToggle(next: boolean) {
    if (next && !data?.ai_consent_at) { setConsentOpen(true); return; }
    toggle.mutate({ ai_enabled: next }, {
  ...
  function handleAcceptAndEnable() {
    setAccepting(true);
    toggle.mutate({ ai_enabled: true, accept_consent: true }, {
      onSuccess: () => { toast.success("Termo aceito. IA Aura ativada"); setConsentOpen(false); setAccepting(false); },
      onError: (e: any) => { toast.error(e?.data?.error || "Erro ao ativar"); setAccepting(false); },
    });
  }
    ...
    <>
      <Card>
        ...
        <View style={{
          ...
          <View style={{ flex: 1 }}>
            ...
            <Text style={{ fontSize: 10, color: DentalColors.ink3 }}>
              {data.ai_enabled ? "Brief, sugestões, Q&A e resumo automático rodam no Modo Consulta." : "Ative para liberar IA no Modo Consulta da clínica."}
            </Text>
          </View>
          <Switch value={data.ai_enabled} onValueChange={handleToggle} disabled={toggle.isPending}
            trackColor={{ false: DentalColors.border, true: DentalColors.cyan }} thumbColor="#fff" />
        </View>
        ...
        {data.ai_enabled ? (
          <View style={{ marginTop: 10, padding: 12, backgroundColor: DentalColors.bg2, borderRadius: 8, borderWidth: 1, borderColor: DentalColors.border }}>
            <View style={{ flexDirection: "row", justifyContent: "space-between", marginBottom: 6 }}>
              <Text style={{ fontSize: 10, fontWeight: "700", color: DentalColors.cyan, letterSpacing: 1 }}>USO ESTE MÊS</Text>
              <Text style={{ fontSize: 10, color: DentalColors.ink3 }}>
            ...
            <View style={{ height: 6, backgroundColor: DentalColors.surface, borderRadius: 3, overflow: "hidden" }}>
              <View style={{ height: "100%", width: `${usagePct}%`,
        ...
        {data.ai_consent_outdated ? (
          <View style={{ marginTop: 8, padding: 10, backgroundColor: "rgba(251,191,36,0.06)", borderRadius: 6, borderWidth: 1, borderColor: "rgba(251,191,36,0.30)" }}>
            <Text style={{ fontSize: 11, color: DentalColors.amber, fontWeight: "600", marginBottom: 4 }}>⚠ Termo de uso atualizado</Text>
            <Text style={{ fontSize: 10, color: DentalColors.ink2 }}>
              Houve uma nova versão do termo. Reaceite para continuar usando.
            </Text>
            <Pressable onPress={() => acceptConsent.mutate()} disabled={acceptConsent.isPending}
              style={{ marginTop: 8, alignSelf: "flex-start", backgroundColor: DentalColors.amber, paddingHorizontal: 10, paddingVertical: 5, borderRadius: 6 }}>
      ...
      <Modal visible={consentOpen} animationType="fade" transparent onRequestClose={() => setConsentOpen(false)}>
        <View style={{ flex: 1, backgroundColor: "rgba(0,0,0,0.7)", justifyContent: "center", padding: 20 }}>
          <View style={{ backgroundColor: DentalColors.bg2, borderRadius: 16, borderWidth: 1, borderColor: DentalColors.border, maxHeight: "85%", padding: 20 }}>
            <Text style={{ fontSize: 18, fontWeight: "800", color: DentalColors.ink, marginBottom: 6 }}>Termo de uso · IA Aura</Text>
            ...
            <ScrollView style={{ maxHeight: 320, padding: 12, backgroundColor: DentalColors.bg, borderRadius: 8, borderWidth: 1, borderColor: DentalColors.border }}>
              <Text style={{ fontSize: 12, color: DentalColors.ink, lineHeight: 18 }}>{data.consent_text}</Text>
            </ScrollView>
            <View style={{ flexDirection: "row", gap: 8, justifyContent: "flex-end", marginTop: 14 }}>
              <Pressable onPress={() => setConsentOpen(false)} style={{ paddingHorizontal: 14, paddingVertical: 8, borderRadius: 8, borderWidth: 1, borderColor: DentalColors.border }}>
                <Text style={{ color: DentalColors.ink2, fontSize: 12, fontWeight: "600" }}>Cancelar</Text>
              </Pressable>
              <Pressable onPress={handleAcceptAndEnable} disabled={accepting}
                style={{ paddingHorizontal: 18, paddingVertical: 10, borderRadius: 8, backgroundColor: DentalColors.cyan, opacity: accepting ? 0.7 : 1, flexDirection: "row", alignItems: "center", gap: 6 }}>
                {accepting ? <ActivityIndicator color="#fff" size="small" /> : null}
                <Text style={{ color: "#fff", fontSize: 12, fontWeight: "700" }}>Aceitar e ativar IA</Text>
              </Pressable>
            </View>
          </View>
        </View>
...
function Header() {
  return (
    <View style={{ marginBottom: 4 }}>
      <Text style={{ fontSize: 9, color: DentalColors.cyan, fontWeight: "700", letterSpacing: 1.4, textTransform: "uppercase", marginBottom: 4 }}>✨ IA Aura</Text>
      <Text style={{ fontSize: 16, fontWeight: "700", color: DentalColors.ink }}>Assistente clínica no Modo Consulta</Text>
...
function Card({ children }: { children: React.ReactNode }) {
  return (
    <View style={{ backgroundColor: DentalColors.surface, borderRadius: 14, borderWidth: 1, borderColor: DentalColors.border, padding: 18, marginBottom: 18 }}>
      {children}
    </View>
  );
}
