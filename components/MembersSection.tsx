    ...
    <View>
      <Pressable onPress={function() { if (canEdit && !isPending) setExpanded(!expanded); }} style={s.memberRow}>
        ...
        <View style={{ flexDirection: "row", alignItems: "center", gap: 8 }}>
          ...
          {canEdit && !isPending && <Icon name={expanded ? "chevron_up" : "chevron_down"} size={14} color={Colors.ink3} />}
        </View>
      </Pressable>
      ...
      <ConfirmDialog visible={confirmRemove}
...
function InviteSuccessCard({ inviteUrl, email, role, companyName, onClose }: {
  ...
  return (
    <View style={s.inviteSuccess}>
      ...
      <View style={s.inviteLinkBox}><Text style={s.inviteLinkText} numberOfLines={2}>{inviteUrl}</Text></View>
      <View style={{ flexDirection: "row", gap: 8, marginTop: 10 }}>
        <Pressable onPress={copyLink} style={s.copyBtn}><Icon name="copy" size={13} color={Colors.violet3} /><Text style={s.copyBtnText}>Copiar link</Text></Pressable>
        <Pressable onPress={shareWhatsApp} style={s.whatsappBtn}><Icon name="message" size={13} color={Colors.green} /><Text style={s.whatsappBtnText}>WhatsApp</Text></Pressable>
      </View>
...
export function MembersSection() {
  ...
  async function handleInviteEmail() {
    ...
    try { await inviteMember({ email: inviteEmail.trim().toLowerCase(), role_label: inviteRole }); setInviteMode("none"); setInviteEmail(""); setInviteRole("Colaborador"); } catch {}
  }
  ...
  return (
    <View style={s.container}>
      ...
      {lastInvite && <InviteSuccessCard inviteUrl={lastInvite.url} email={lastInvite.email} role={lastInvite.role} companyName={company?.name || "Aura"} onClose={clearLastInvite} />}

      {inviteMode === "link" && (
        <View style={s.inviteForm}>
          <Text style={s.formTitle}>Gerar link de acesso</Text>
          <Text style={{ fontSize: 12, color: Colors.ink3, marginBottom: 14, lineHeight: 18 }}>Um link unico sera gerado. Qualquer pessoa com o link pode criar uma conta e entrar na sua equipe.</Text>
          <RoleSelector />
      ...
      {inviteMode === "email" && (
        <View style={s.inviteForm}>
          ...
          <TextInput style={[s.input, inviteEmail && !emailValid && s.inputError]} value={inviteEmail} onChangeText={setInviteEmail}
            placeholder="email@exemplo.com" placeholderTextColor={Colors.ink3} autoCapitalize="none" keyboardType="email-address" />
          {inviteEmail && !emailValid && <Text style={s.fieldError}>E-mail invalido</Text>}
          <View style={{ marginTop: 14 }}><RoleSelector /></View>
          <View style={{ flexDirection: "row", gap: 8 }}>
            <Pressable onPress={cancelInvite} style={s.cancelBtn}><Text style={s.cancelBtnText}>Cancelar</Text></Pressable>
            <Pressable onPress={handleInviteEmail} disabled={isInviting || !emailValid} style={[s.sendBtn, (isInviting || !emailValid) && { opacity: 0.5 }]}>
      ...
      {isLoading ? (
        <View style={{ alignItems: "center", paddingVertical: 20 }}><ActivityIndicator color={Colors.violet3} /></View>
      ) : members.length === 0 ? (
      ...
      {members.length > 0 && (
        <View style={s.billingNote}><Text style={s.billingText}>Titular nao cobrado. Cada membro adicional: R$19/mes.</Text></View>
      )}
    </View>
  );
}
