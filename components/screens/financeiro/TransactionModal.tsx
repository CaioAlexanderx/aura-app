...
// Mini cart item pra venda retroativa
type SaleItem = {
  cartKey: string;
  productId: string;
  variantId?: string;
...
export function TransactionModal({ visible, onClose, onSave, onSaleCreated }: {
  ...
  // --- Sale (Venda retroativa) ---
  var saleFiltered = products.filter(function(p) {
    if (!saleSearch || saleSearch.length < 2) return false;
    return p.name.toLowerCase().includes(saleSearch.toLowerCase());
  }).slice(0, 8);
  ...
  function handleAddSaleProduct(product: any) {
    if (product.has_variants) {
      ...
      companiesApi.variants(company!.id, product.id)
        .then(function(res) { setVariantOptions((res.variants || []).filter(function(v: any) { return v.is_active !== false; })); })
        .catch(function() { setVariantOptions([]); })
        .finally(function() { setVariantLoading(false); });
    } else {
      addSaleItem(product.id, null, product.name, product.price);
      setSaleSearch("");
    }
  }
  ...
  function handleSelectVariant(v: any) {
    ...
    var label = attrs.map(function(a: any) {
      if (/^#[0-9a-fA-F]{6}$/.test(a.value)) return hexToName(a.value);
      return a.value;
  ...
  function addSaleItem(productId: string, variantId: string | null, name: string, price: number) {
    ...
    setSaleItems(function(prev) {
      var existing = prev.find(function(i) { return i.cartKey === key; });
      if (existing) return prev.map(function(i) { return i.cartKey === key ? { ...i, qty: i.qty + 1 } : i; });
      return [...prev, { cartKey: key, productId: productId, variantId: variantId || undefined, name: name, price: price, qty: 1 }];
    });
  }

  function removeSaleItem(key: string) {
    setSaleItems(function(prev) { return prev.filter(function(i) { return i.cartKey !== key; }); });
  }

  function updateSaleQty(key: string, delta: number) {
    setSaleItems(function(prev) { return prev.map(function(i) {
  ...
  async function handleSaveSale() {
    ...
    setSaleSaving(true);
    try {
      await pdvApi.createSale(company.id, {
        items: saleItems.map(function(i) {
          return { product_id: i.productId, variant_id: i.variantId || undefined, quantity: i.qty, unit_price: i.price, product_name_snapshot: i.name };
        }),
    ...
    <View style={s.overlay}>
      <View style={[s.modal, isSale && { maxWidth: 540 }]}>
        ...
        <View style={s.toggleRow}>
          ...
          <Pressable onPress={function() { setTxType("sale"); }} style={[s.toggleBtn, isSale && { backgroundColor: Colors.violetD, borderColor: Colors.violet }]}><Text style={[s.toggleText, isSale && { color: Colors.violet3 }]}>Venda</Text></Pressable>
        </View>

        {/* === RECEITA / DESPESA (existente) === */}
        {!isSale && (
          <>
        ...
        {isSale && (
          <ScrollView style={{ maxHeight: 480 }} contentContainerStyle={s.form}>
            ...
            <View style={s.rowFields}>
              <View style={{ width: 130 }}>
                <Text style={s.label}>Data da venda</Text>
                <TextInput style={s.input} value={dateStr} onChangeText={function(v) { setDateStr(maskDate(v)); }} placeholder="DD/MM/AAAA" placeholderTextColor={Colors.ink3} keyboardType="number-pad" maxLength={10} />
              </View>
              <View style={{ flex: 1 }}>
                ...
                <View style={{ flexDirection: "row", gap: 4, flexWrap: "wrap" }}>
                  {PAYMENTS.map(function(p) {
                    return <Pressable key={p.key} onPress={function() { setSalePayment(p.key); }} style={[s.catBtn, salePayment === p.key && s.catBtnActive]}><Text style={[s.catText, salePayment === p.key && s.catTextActive]}>{p.label}</Text></Pressable>;
                  })}
                </View>
              </View>
            </View>
            ...
            {/* Selecao de variante inline */}
            {variantPending && (
              <View style={s.variantBlock}>
                <View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
                  <Text style={s.variantTitle}>Selecione a variante de "{variantPending.name}"</Text>
                  <Pressable onPress={function() { setVariantPending(null); setVariantOptions([]); }}><Text style={{ fontSize: 12, color: Colors.red }}>Cancelar</Text></Pressable>
                </View>
                {variantLoading ? (
                  <ActivityIndicator color={Colors.violet3} />
                ) : variantOptions.length === 0 ? (
                  <Text style={s.hint}>Nenhuma variante encontrada</Text>
                ) : (
                  <View style={{ gap: 6 }}>
                    {variantOptions.map(function(v: any) {
                      var attrs = v.attributes || [];
                      var label = attrs.map(function(a: any) { return /^#[0-9a-fA-F]{6}$/.test(a.value) ? hexToName(a.value) : a.value; }).filter(Boolean).join(" \u00b7 ") || v.sku_suffix;
                      var stock = parseInt(v.stock_qty) || 0;
                      var hex = attrs.find(function(a: any) { return /^#/.test(a.value); })?.value;
                      return (
                        <Pressable key={v.id} onPress={function() { handleSelectVariant(v); }} style={s.variantOption}>
                          {hex && <View style={[s.srColor, { backgroundColor: hex, width: 20, height: 20 }]} />}
                          <Text style={s.srName}>{label}</Text>
                          <Text style={s.srMeta}>{stock} un</Text>
                        </Pressable>
                      );
                    })}
                  </View>
            ...
            {saleItems.length > 0 && (
              <View style={s.miniCart}>
                <Text style={s.label}>Itens da venda</Text>
                {saleItems.map(function(item) {
                  return (
                    <View key={item.cartKey} style={s.miniCartRow}>
                      <View style={{ flex: 1 }}>
                        <Text style={s.srName} numberOfLines={1}>{item.name}</Text>
                        <Text style={s.srMeta}>{fmtPrice(item.price)} x {item.qty} = {fmtPrice(item.price * item.qty)}</Text>
                      </View>
                      <View style={{ flexDirection: "row", alignItems: "center", gap: 4 }}>
                        <Pressable onPress={function() { updateSaleQty(item.cartKey, -1); }} style={s.qtyBtn}><Text style={s.qtyBtnText}>-</Text></Pressable>
                        <Text style={s.qtyText}>{item.qty}</Text>
                        <Pressable onPress={function() { updateSaleQty(item.cartKey, 1); }} style={s.qtyBtn}><Text style={s.qtyBtnText}>+</Text></Pressable>
                        <Pressable onPress={function() { removeSaleItem(item.cartKey); }} style={s.removeBtn}><Text style={s.removeText}>x</Text></Pressable>
                      </View>
                    </View>
                  );
                })}
                <View style={{ flexDirection: "row", justifyContent: "space-between", marginTop: 10, paddingTop: 10, borderTopWidth: 1, borderTopColor: Colors.border }}>
                  <Text style={{ fontSize: 14, fontWeight: "600", color: Colors.ink }}>Total</Text>
                  <Text style={{ fontSize: 16, fontWeight: "800", color: Colors.green }}>{fmtPrice(saleTotal)}</Text>
                </View>
              </View>
            )}

            <Pressable onPress={handleSaveSale} disabled={saleSaving || saleItems.length === 0}
              style={[s.saveBtn, { backgroundColor: Colors.violet, opacity: saleSaving || saleItems.length === 0 ? 0.5 : 1 }]}>
              {saleSaving ? <ActivityIndicator color="#fff" size="small" /> : <Text style={s.saveBtnText}>Registrar venda retroativa</Text>}
            </Pressable>
          </ScrollView>
        )}
      </View>
