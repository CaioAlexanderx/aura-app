...
export var PERIOD_LABELS: Record<string, string> = { week: "Semanal", month: "Mensal", year: "Anual", prev_year: "Ano Anterior" };
...
export function SectionTitle({ title, hint }: { title: string; hint?: string }) {
  return <View style={{ marginBottom: 10 }}><Text style={cs.secTitle}>{title}</Text>{hint && <Text style={cs.secHint}>{hint}</Text>}</View>;
}

export function HoverBar({ height, maxH, value, label, color, width, showValue }: { height: number; maxH: number; value: string; label: string; color: string; width?: number; showValue?: boolean }) {
  var [h, sH] = useState(false);
  ...
  return (
    <Pressable onHoverIn={isWeb ? function() { sH(true); } : undefined} onHoverOut={isWeb ? function() { sH(false); } : undefined}
      style={{ alignItems: "center", gap: 2, width: w, position: "relative" as any }}>
      {h && <View style={cs.tooltip}><Text style={cs.tooltipText}>{value}</Text></View>}
      {showValue && !h && <Text style={cs.barValueText}>{value}</Text>}
      <View style={[cs.chartBar, { height: barH, backgroundColor: color, opacity: h ? 1 : 0.85 }, isWeb && { transition: "height 0.4s ease, opacity 0.2s ease" } as any]} />
...
export var cs = StyleSheet.create({
  ...
  barValueText: { fontSize: 8, color: Colors.ink3, fontWeight: "600", textAlign: "center" },
  tooltip: { position: "absolute" as any, top: -30, backgroundColor: Colors.bg, borderRadius: 6, paddingHorizontal: 8, paddingVertical: 4, borderWidth: 1, borderColor: Colors.border2, zIndex: 10 },
  ...
  dowRow: { flexDirection: "row", alignItems: "center", gap: 8, paddingVertical: 5, paddingHorizontal: 6, borderRadius: 8 },
