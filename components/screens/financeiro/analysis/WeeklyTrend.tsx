...
export function WeeklyTrend({ data }: { data: any[] }) {
  ...
  return <Card style={{ marginBottom: 16 }}>
    ...
    <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ flexDirection: "row", alignItems: "flex-end", gap: 4, paddingVertical: 8, minHeight: CHART_H - 40 }}>
      {data.map(function(w, i) {
        return <HoverBar key={i} height={w.faturamento} maxH={maxF} value={fmtK(w.faturamento) + " (" + w.vendas + " vendas)"} label={w.semana} color={PAL.violet} width={48} />;
      })}
    </ScrollView>
  </Card>;
}
