// ============================================================
// AURA. — ExpenseDetail
// Visualizacao detalhada de despesas para a aba Visao Geral
// (corrige a lacuna: ate aqui so havia visao detalhada de receitas)
// ============================================================
...
// Paleta despesa: tons quentes com violeta de acento (Claude Design)
var EXPENSE_PALETTE = [
  "#f87171", // coral red
  "#fb923c", // amber-orange
  "#fbbf24", // amber
...
export function ExpenseDetail({ transactions, previousExpenses }: Props) {
  var data = useMemo(function() {
    ...
    // --- Top 5 individuais ---
    var top = expenses.slice().sort(function(a, b) { return b.amount - a.amount; }).slice(0, 5);

    // --- Tendencia diaria (ultimos dias com lancamento) ---
    ...
    expenses.forEach(function(t) {
      ...
      var raw = t.due_date || t.created_at || "";
      // FIX timezone: parseDateLocal evita classificar pagamento de hoje
      // como overdue por causa do recuo de 1 dia em BRT.
    ...
    var paid = expenses.filter(function(t) { return t.status === "confirmed"; }).reduce(function(s, t) { return s + t.amount; }, 0);

    return { total: total, avg: avg, count: expenses.length, cats: cats, top: top, daily: daily, buckets: buckets, totalPending: totalPending, methods: methods, paid: paid };
  }, [transactions]);
  ...
  if (!data) {
    return (
      <View style={s.empty}>
        ...
        <Text style={s.emptyTitle}>Nenhuma despesa no periodo</Text>
        <Text style={s.emptyHint}>Quando você lançar despesas, esta área trará o detalhamento completo.</Text>
      </View>
    );
  }

  var delta: number | null = previousExpenses != null && previousExpenses > 0
    ? ((data.total - previousExpenses) / previousExpenses) * 100
    : null;
  ...
  return (
    <View style={s.container}>
      ...
      <View style={s.headerCard}>
        {isWeb ? (
          <div style={{ position: "absolute", top: -90, right: -90, width: 260, height: 260, borderRadius: "50%", background: "radial-gradient(circle, rgba(248,113,113,0.22), transparent 70%)", pointerEvents: "none" } as any} />
        ) : (
        ...
        <View style={s.headerContent}>
          <View style={s.headerRow}>
            <View style={{ flex: 1 }}>
              <Text style={s.headerLabel}>Despesas no periodo</Text>
              <Text style={s.headerValue}>{fmt(data.total)}</Text>
            </View>
            {delta != null && (
              <View style={[s.deltaPill, delta >= 0 ? s.deltaPillUp : s.deltaPillDown]}>
                <Text style={[s.deltaPillText, { color: delta >= 0 ? Colors.red : Colors.green }]}>
          ...
          <View style={s.headerStrip}>
            ...
            <View style={s.headerStripDivider} />
            <View style={s.headerStripCell}>
              <Text style={s.headerStripLabel}>Pago</Text>
              <Text style={[s.headerStripValue, { color: Colors.green }]}>{fmtK(data.paid)}</Text>
            </View>
            <View style={s.headerStripDivider} />
            <View style={s.headerStripCell}>
              <Text style={s.headerStripLabel}>Pendente</Text>
      ...
      <View style={s.card}>
        <View style={s.cardHead}>
          <View style={{ flex: 1 }}>
            <Text style={s.cardTitle}>Categorias de despesa</Text>
            <Text style={s.cardHint}>Onde o dinheiro esta saindo</Text>
          </View>
      ...
      {data.daily.length >= 3 && (
        <View style={s.card}>
          <View style={s.cardHead}>
            <View style={{ flex: 1 }}>
              <Text style={s.cardTitle}>Despesas por dia</Text>
      ...
      <View style={s.card}>
        <View style={s.cardHead}>
          <View style={{ flex: 1 }}>
            <Text style={s.cardTitle}>Maiores despesas</Text>
            <Text style={s.cardHint}>Top 5 lancamentos por valor</Text>
          </View>
        </View>
        <TopExpenses top={data.top} max={data.top[0]?.amount || 1} />
      </View>
      ...
      {data.totalPending > 0 && (
        <View style={s.card}>
          <View style={s.cardHead}>
            <View style={{ flex: 1 }}>
              <Text style={s.cardTitle}>A pagar — linha do tempo</Text>
              <Text style={s.cardHint}>Distribuicao das despesas pendentes</Text>
            </View>
            <View style={[s.cardBadge, { backgroundColor: Colors.amberD, borderColor: Colors.amber + "40" }]}>
              <Text style={[s.cardBadgeText, { color: Colors.amber }]}>{fmtK(data.totalPending)}</Text>
            </View>
          </View>
          <PayableTimeline buckets={data.buckets} total={data.totalPending} />
        </View>
      )}
      ...
      {data.methods.length > 0 && (
        <View style={s.card}>
          <View style={s.cardHead}>
            <View style={{ flex: 1 }}>
              <Text style={s.cardTitle}>Formas de pagamento</Text>
              <Text style={s.cardHint}>Como as despesas foram pagas</Text>
            </View>
          </View>
...
function CategorySection({ cats, total }: { cats: { name: string; total: number; count: number; pct: number }[]; total: number }) {
    ...
    return (
      <View style={{ marginTop: 12, gap: 12 }}>
        {cats.map(function(c, i) {
          var clr = EXPENSE_PALETTE[i % EXPENSE_PALETTE.length];
          return (
            <View key={c.name}>
  ...
  var slices = cats.map(function(c, i) {
    ...
    return `<circle cx="${cx}" cy="${cy}" r="${r}" fill="none" stroke="${EXPENSE_PALETTE[i % EXPENSE_PALETTE.length]}" stroke-width="${sw}" stroke-dasharray="${dash} ${gap}" stroke-dashoffset="${-o}" opacity="0.92"/>`;
  }).join("");
  var svg = `<svg width="${size}" height="${size}" viewBox="0 0 ${size} ${size}" style="transform:rotate(-90deg);filter:drop-shadow(0 4px 12px rgba(248,113,113,0.18))">
...
function DailyTrend({ daily }: { daily: { key: string; label: string; value: number; dayOfMonth: number }[] }) {
          ...
          {daily.map(function(d) {
            ...
            return (
              <View key={d.key} style={{ alignItems: "center", gap: 4, width: 22 }}>
                <View style={{ height: h, width: 14, borderRadius: 4, backgroundColor: "#f87171", opacity: 0.85 }} />
  ...
  var bars = daily.map(function(d, idx) {
    ...
    return `<g>
      <rect x="${bx}" y="${by}" width="${barW}" height="${h}" rx="3" fill="url(#expGrad${isPeak ? "Peak" : (idx % 2 === 0 ? "A" : "B")})">
        <title>${d.label}: ${fmt(d.value)}</title>
      </rect>
      ${isPeak ? `<circle cx="${bx + barW / 2}" cy="${by - 4}" r="2.5" fill="#f87171"/>` : ""}
    </g>`;
  }).join("");
  ...
  var defs = `<defs>
    <linearGradient id="expGradPeak" x1="0" y1="0" x2="0" y2="1">
    ...
    <linearGradient id="expGradB" x1="0" y1="0" x2="0" y2="1">
      <stop offset="0%" stop-color="#fb923c" stop-opacity="0.85"/>
      <stop offset="100%" stop-color="#fb923c" stop-opacity="0.45"/>
    </linearGradient>
  </defs>`;
    ...
    <View style={{ marginTop: 10 }}>
      ...
      <View style={dt.legend}>
        <View style={dt.legendItem}>
          <View style={[dt.legendDot, { backgroundColor: "#f87171" }]} />
          <Text style={dt.legendText}>Despesa diaria</Text>
        </View>
        <View style={dt.legendItem}>
...
// ============================================================
function TopExpenses({ top, max }: { top: Transaction[]; max: number }) {
  return (
    <View style={{ marginTop: 12, gap: 12 }}>
              ...
              <View style={topS.metaRow}>
                ...
                {t.status === "pending" && (
                  <View style={topS.metaPendingPill}>
                    <Text style={topS.metaPendingText}>pendente</Text>
                  </View>
                )}
              </View>
            </View>
...
// ============================================================
// SUB: A pagar timeline — stacked bar + grid de buckets
// ============================================================
type BucketsT = {
...
function PayableTimeline({ buckets, total }: { buckets: BucketsT; total: number }) {
  var items = [
    { key: "overdue", label: "Atrasadas", hint: "ja vencidas", color: "#f87171", data: buckets.overdue },
    { key: "week", label: "Esta semana", hint: "prox. 7 dias", color: "#fb923c", data: buckets.week },
    { key: "month", label: "Este mes", hint: "ainda neste mes", color: "#fbbf24", data: buckets.month },
...
// ============================================================
// SUB: Formas de pagamento — barras horizontais
// ============================================================
...
var s = StyleSheet.create({
  ...
  headerOrb: { position: "absolute", top: -80, right: -80, width: 220, height: 220, borderRadius: 110, backgroundColor: "rgba(248,113,113,0.10)" } as any,
  ...
  deltaPillUp: { backgroundColor: Colors.redD, borderColor: Colors.red + "40" },
  deltaPillDown: { backgroundColor: Colors.greenD, borderColor: Colors.green + "40" },
...
var topS = StyleSheet.create({
  ...
  rankFirst: { backgroundColor: "rgba(248,113,113,0.12)", borderColor: "rgba(248,113,113,0.35)" },
  rankNum: { fontSize: 13, color: Colors.ink2, fontWeight: "800" },
  rankNumFirst: { color: "#f87171" },
  body: { flex: 1 },
  ...
  amount: { fontSize: 13, color: Colors.red, fontWeight: "800", ...(isWeb ? ({ fontVariantNumeric: "tabular-nums" } as any) : {}) },
  barBg: { height: 5, borderRadius: 3, backgroundColor: Colors.bg4, overflow: "hidden" },
  barFill: { height: 5, borderRadius: 3, backgroundColor: "#f87171" },
  ...
  metaPendingPill: { paddingHorizontal: 8, paddingVertical: 2, borderRadius: 999, backgroundColor: Colors.amberD, borderWidth: 1, borderColor: Colors.amber + "40" },
  metaPendingText: { fontSize: 9, color: Colors.amber, fontWeight: "700" },
});
