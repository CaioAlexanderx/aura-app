...
// M6: Maximum contrast — each color is a completely different hue
const INCOME_COLORS = ["#059669", "#0891b2", "#7c3aed", "#2563eb", "#ca8a04", "#dc2626"];
...
function AlertStrip({ alerts }: { alerts: { text: string; color: string; icon: string }[] }) {
  ...
  return <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ flexGrow: 0, marginBottom: 20 }} contentContainerStyle={{ flexDirection: "row", gap: 8 }}>{alerts.map((a, i) => <View key={i} style={[s.alertCard, { borderLeftColor: a.color }]}><Text style={[s.alertIcon, { color: a.color }]}>{a.icon}</Text><Text style={s.alertText}>{a.text}</Text></View>)}</ScrollView>;
}

function NarrativeBlock({ insights }: { insights: string[] }) {
  if (insights.length === 0) return null;
  return <View style={s.narrativeCard}><Text style={s.narrativeTitle}>Por que esse resultado?</Text>{insights.map((ins, i) => <View key={i} style={s.narrativeRow}><Text style={s.narrativeBullet}>{"\u2022"}</Text><Text style={s.narrativeText}>{ins}</Text></View>)}</View>;
}
...
function CategoryDonut({ title, items, total, colors }: { title: string; items: { category: string; amount: number }[]; total: number; colors: string[] }) {
  ...
  return (
    <View style={s.donutSection}><Text style={s.donutTitle}>{title}</Text><View style={s.donutCard}><View style={s.donutRow}>
      <DonutChart items={items} total={total} colorFn={(i) => colors[i % colors.length]} />
      <View style={s.donutLegend}>{items.map((item, i) => { const pct = total > 0 ? Math.round((item.amount / total) * 100) : 0; return <View key={item.category} style={s.legendItem}><View style={[s.legendDot, { backgroundColor: colors[i % colors.length] }]} /><View style={{ flex: 1 }}><Text style={s.legendLabel}>{item.category}</Text><Text style={s.legendValue}>{fmt(item.amount)} ({pct}%)</Text></View></View>; })}</View>
    </View></View></View>
  );
}

function KpiStrip({ kpis }: { kpis: { label: string; value: string; color?: string; sub?: string }[] }) {
  return <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ flexGrow: 0, marginBottom: 20 }} contentContainerStyle={{ flexDirection: "row", gap: 8 }}>{kpis.map(k => <View key={k.label} style={s.kpiCard}><Text style={s.kpiLabel}>{k.label}</Text><Text style={[s.kpiValue, k.color ? { color: k.color } : {}]}>{k.value}</Text>{k.sub && <Text style={s.kpiSub}>{k.sub}</Text>}</View>)}</ScrollView>;
}

export function TabResumo({ transactions, dreApi }: Props) {
  const computed = useMemo(() => {
    if (transactions.length === 0) return null;
    const incomeMap: Record<string, number> = {}; const expenseMap: Record<string, number> = {};
    let totalIncome = 0, totalExpenses = 0;
    transactions.forEach(t => { if (t.type === "income") { incomeMap[t.category] = (incomeMap[t.category] || 0) + t.amount; totalIncome += t.amount; } else { expenseMap[t.category] = (expenseMap[t.category] || 0) + t.amount; totalExpenses += t.amount; } });
    ...
    const balance = totalIncome - totalExpenses; const marginPct = totalIncome > 0 ? Math.round((balance / totalIncome) * 100) : 0;
    const incomeCount = transactions.filter(t => t.type === "income").length; const expenseCount = transactions.filter(t => t.type === "expense").length;
    ...
    if (incomeCount > 0) { if (incomeCount <= 3) insights.push(`Com apenas ${incomeCount} vendas, seu modelo depende de poucas transacoes com ticket alto (${fmt(avgTicket)}).`); else insights.push(`${incomeCount} vendas com ticket medio de ${fmt(avgTicket)}.`); }
    if (balance < 0 && incomeCount > 0 && avgTicket > 0) { const extra = Math.ceil(Math.abs(balance) / avgTicket); insights.push(`Se aumentar em ${extra} vendas, o resultado tende a virar positivo.`); }
    ...
    return { totalIncome, totalExpenses, balance, marginPct, incomeItems, expenseItems, alerts, insights, kpis };
  }, [transactions]);
  ...
  return (
    <View>
      ...
      <AlertStrip alerts={computed.alerts} /><KpiStrip kpis={computed.kpis} /><NarrativeBlock insights={computed.insights} />
