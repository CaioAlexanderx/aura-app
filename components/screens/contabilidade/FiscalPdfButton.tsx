...
var PDF_ENDPOINTS: Record<string, { path: string; label: string; icon: string }> = {
  das_mei: { path: "/obligations/das-mei/pdf", label: "Baixar resumo DAS-MEI", icon: "file_text" },
  das_sn: { path: "/obligations/das-sn/pdf", label: "Baixar demonstrativo DAS", icon: "file_text" },
  dasn_simei: { path: "/obligations/dasn/report", label: "Baixar relatorio anual", icon: "file_text" },
  pgdas_d: { path: "/obligations/das-sn/pdf", label: "Baixar demonstrativo DAS", icon: "file_text" },
};

// Qual step de cada obrigacao mostra o botao de PDF
var STEP_MAP: Record<string, number> = {
  das_mei: 0,
  das_sn: 2,
  dasn_simei: 0,
  pgdas_d: 0,
...
export function FiscalPdfButton({ obligationCode, stepIndex, completed }: Props) {
  ...
  var config = PDF_ENDPOINTS[obligationCode];
  var targetStep = STEP_MAP[obligationCode];

  // So mostra no step correto e se nao esta completo
  if (!config || stepIndex !== targetStep || completed) return null;
  ...
  function handleOpen() {
    ...
    if (isWeb && typeof window !== "undefined") {
      // Abre em nova aba com auth header via fetch + blob
