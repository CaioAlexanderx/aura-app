...
/**
 * Converts hex color to human-readable Portuguese name.
 * Uses nearest-color matching in RGB space against a curated palette
 * (the 20 most common colors in retail — fashion/varejo focus).
 */
function hexToName(hex: string | undefined): string {
  ...
  var palette: { name: string; r: number; g: number; b: number }[] = [
    ...
    { name: "Bege",      r: 240, g: 235, b: 223 },
    { name: "Nude",      r: 226, g: 197, b: 170 },
    { name: "Marrom",    r: 139, g: 69,  b: 19  },
    { name: "Marrom",    r: 189, g: 113, b: 0   }, // caramelo/terracota
    { name: "Caramelo",  r: 196, g: 138, b: 92  },
    { name: "Vermelho",  r: 220, g: 20,  b: 20  },
    { name: "Vinho",     r: 114, g: 28,  b: 36  },
    { name: "Rosa",      r: 255, g: 105, b: 180 },
    { name: "Rosa claro",r: 255, g: 182, b: 193 },
    { name: "Laranja",   r: 255, g: 140, b: 0   },
    { name: "Amarelo",   r: 255, g: 215, b: 0   },
    { name: "Verde",     r: 34,  g: 139, b: 34  },
    { name: "Verde claro",r: 139, g: 232, b: 179 },
    { name: "Verde agua",r: 64,  g: 224, b: 208 },
    { name: "Azul",      r: 30,  g: 100, b: 200 },
    { name: "Azul marinho",r: 25, g: 25, b: 112 },
    { name: "Azul claro",r: 135, g: 206, b: 235 },
    { name: "Roxo",      r: 128, g: 0,   b: 128 },
    { name: "Lilas",     r: 200, g: 162, b: 200 },
...
export function ProductCard({ product, onAdd, isWide }: { product: { id: string; name: string; price: number; category: string; stock: number; barcode: string; color?: string; size?: string }; onAdd: () => void; isWide: boolean }) {
  ...
  return (
    <Pressable onPress={onAdd} onHoverIn={isWeb ? function() { sH(true); } : undefined} onHoverOut={isWeb ? function() { sH(false); } : undefined}
      ...
      {/* Variant row — bigger color chip + color name + size badge */}
...
var s = StyleSheet.create({
  ...
  // Barcode tail — last 4 digits, prominent & monospaced (unique identifier hint)
  barcodeTailWrap: {
    flexDirection: "row",
    alignItems: "center",
  ...
  // Variant row — clear color + name + size
