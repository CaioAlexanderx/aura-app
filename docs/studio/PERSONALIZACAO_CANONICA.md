...
- **`components/studio/visualEngine/PersonalizacaoLivePreview.tsx` foi apagado.** Ficou órfão com o fim do wizard — era o único uso que existia dele — e não dá para reaproveitar no painel: lia o `productId` de `useLocalSearchParams`, o que só funciona numa rota `produtos/[id]`, e o painel monta no drawer do estoque e no `StudioNewProductWizard`; recebia `widthCm`/`heightCm`/`position`, a forma do wizard que este trabalho eliminou; e passava `values` com as chaves literais `text`/`image`, em vez do `previewValues` por id de campo que o painel monta. As três coisas que ele tinha a mais que o `EnginePreview` **não se perderam de vista**: o outline da área em cm é a opção `showAreas` que `composeView` já aceita; o export HD 2048px é `exportPng`, vivo e em uso pelo `gerarRenderAprovacao`; e o toggle frente/verso já existe em versão melhor no `components/studio/storefront/LivePreview.tsx`, que gateia por `config.has_back` em vez de só contar as vistas. Quem quiser esses três no painel liga no `EnginePreview`, que é o preview canônico.
