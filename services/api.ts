...
export var billingApi = {
  ...
  tokenize: function(companyId: string, cardData: { card_number: string; card_expiry_month: string; card_expiry_year: string; card_ccv: string; holder_name: string; holder_cpf: string; holder_postal_code?: string; holder_address_number?: string; holder_address?: string }) {
    return request<TokenizeResponse>("/companies/" + companyId + "/billing/tokenize", { method: "POST", body: cardData, retry: 0, timeout: 15000 });
  },
  subscribe: function(companyId: string, plan: string, billingType?: string, cycle?: string, opts?: { creditCardToken?: string; holderName?: string; holderCpf?: string; holderPostalCode?: string; holderAddressNumber?: string; holderAddress?: string; endDate?: string; totalCycles?: number }) {
    return request<SubscribeResponse>("/companies/" + companyId + "/billing/subscribe", { method: "POST", body: { plan: plan, billing_type: billingType || "PIX", cycle: cycle || "monthly", end_date: opts?.endDate, total_cycles: opts?.totalCycles, credit_card_token: opts?.creditCardToken, credit_card_holder_name: opts?.holderName, credit_card_holder_cpf: opts?.holderCpf, credit_card_holder_postal_code: opts?.holderPostalCode, credit_card_holder_address_number: opts?.holderAddressNumber, credit_card_holder_address: opts?.holderAddress } });
  },
