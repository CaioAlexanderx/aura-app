...
export const cnpjApi = {
  lookup: (cnpj: string) => request<any>("/onboarding/cnpj-lookup", { method: "POST", body: { cnpj }, retry: 1 }),
};
...
export const aiApi = {
  ...
  activity: (companyId: string, limit?: number) =>
    request<{ activity: any[]; summary: any[]; total: number }>(`/companies/${companyId}/ai/activity?limit=${limit || 20}`),
};
