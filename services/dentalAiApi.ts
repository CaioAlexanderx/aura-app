...
export interface AiConsultaRequest {
  ...
  patientId: string;
  query?: string;
  context?: { transcripts?: string[]; toothChanges?: AiToothChange[]; };
}
...
export interface AiConsultaResponse {
  ...
  cost_usd: number;
  latency_ms: number;
  quota: { used: number; total: number | null; remaining: number | null; unlimited?: boolean; };
  simulated?: boolean;
}
...
export const dentalAiApi = {
  consulta(cid: string, body: AiConsultaRequest) {
    return request<AiConsultaResponse>(`/companies/${cid}/dental/ai/consulta`, {
      method: "POST", body, timeout: 35000, retry: 0,
    });
  },
  usage(cid: string) {
  ...
  acceptConsent(cid: string) {
    return request<{ ok: true; consent_version: string; consent_at: string }>(
      `/companies/${cid}/dental/ai/settings/consent`, { method: "POST" }
    );
  },
};
