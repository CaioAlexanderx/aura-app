...
export interface CpfAnnuity {
  ...
  paid_total?: number;
  total?: number;
  /** F6.5 (13/08/2026, backend) — is_active do PRATICANTE
   *  (customers.is_active), espelhando DojoAnnuity.is_active. Populado
   *  quando practitioner_status=all/inactive traz registros inativos na
   *  mesma lista — a UI precisa disso pra rotular a linha. */
  is_active?: boolean | null;
}
 ...
 *  mostra um número e a lista mostra outro). */
export type AnnuityDojoStatusFilter = DojoStatus | "all";

/** Filtro ativo/inativo por PRATICANTE — F6.5 (13/08/2026, aura-backend):
 *  espelha AnnuityDojoStatusFilter, agora sobre customers.is_active.
 *  GET /financial/annuities/cpf e GET /financial/annuities/summary aceitam
 *  `practitioner_status`: "active" (default do backend) | "inactive" |
 *  "all". Valor inválido → 422. MESMO parâmetro nas duas rotas — o cliente
 *  SEMPRE precisa mandar o mesmo valor pras duas chamadas (senão o topo do
 *  hub mostra um número e a lista de Praticantes mostra outro). Faixa-preta
 *  continua sendo pré-requisito FIXO (não configurável) — este filtro só
 *  muda o critério de ativo/inativo. */
export type AnnuityPractitionerStatusFilter = AffiliationStatus | "all";
...
/** Resposta de PATCH /financial/annuities/cpf/:practitionerId/:annuityId
 *  (edição de anuidade CPF — F6.5, 13/08/2026, aura-backend). Shape PRÓPRIO
 *  dessa rota, análogo a AnnuityHeaderUpdateResult mas com
 *  practitioner_id no lugar de dojo_id (nunca os dois — o header é
 *  dojo_id XOR practitioner_id no backend). `plan` vem sempre 'anual' —
 *  CPF nunca tem outro plano. */
export interface AnnuityCpfHeaderUpdateResult {
  annuity_id: string;
  practitioner_id: string;
  reference_period: string;
  plan: AnnuityPlan | null;
...
/** Body de PATCH /financial/annuities/cpf/:practitionerId/:annuityId
 *  (edição de anuidade CPF — F6.5, 13/08/2026, aura-backend). MUITO mais
 *  simples que AnnuityUpdateInput (dojô): CPF é SEMPRE 1x/ano, plano
 *  'anual' fixo — não existe reestruturação de N parcelas nem troca de
 *  plano aqui. NÃO aceita `plan` nem `installments[]` (só fazem sentido
 *  no dojô, que tem múltiplos regimes) — só amount/due_date/
 *  reference_period. Ao menos um campo obrigatório. SEM lock de status —
 *  funciona mesmo já paga; 422 AMOUNT_BELOW_PAID se o novo valor ficar
 *  abaixo do já recebido. */
export type AnnuityCpfUpdateInput = Partial<{
  amount: number;
  /** 'YYYY-MM-DD' — mesma convenção de due_date usada no resto do módulo. */
  due_date: string;
  reference_period: string;
}>;
...
export const karateApi = {
  ...
  /** Edita uma anuidade CPF (praticante) já lançada — F6.5 (13/08/2026,
   *  aura-backend). Mesma função que updateAnnuity() faz pro dojô, versão
   *  praticante: SEM plan/installments (ver AnnuityCpfUpdateInput), SEM
   *  lock de status (funciona já paga). Resposta NÃO é um CpfAnnuity —
   *  shape próprio (AnnuityCpfHeaderUpdateResult), mesma família de
   *  AnnuityHeaderUpdateResult. */
  updateCpfAnnuity: (
    federationId: string,
    practitionerId: string,
    annuityId: string,
    payload: AnnuityCpfUpdateInput
  ): Promise<AnnuityCpfHeaderUpdateResult> =>
    request(`/federation/${federationId}/financial/annuities/cpf/${practitionerId}/${annuityId}`, {
      method: "PATCH",
      body: payload,
    }),
   ...
   *  F6.5 (13/08/2026) — aceita `practitioner_status` (mesmo leque
   *  active|inactive|all do dojô, ver AnnuityPractitionerStatusFilter);
   *  precisa ser o MESMO valor mandado para getAnnuitySummary, senão a
   *  lista de Praticantes e os KPIs do hub divergem. */
  listCpfAnnuities: (
    federationId: string,
    params?: {
      status?: AnnuityStatusFilter; year?: string; q?: string; page?: number; pageSize?: number;
      practitioner_status?: AnnuityPractitionerStatusFilter;
    }
    ...
    if (params?.page) qs.set("page", String(params.page));
    if (params?.pageSize) qs.set("pageSize", String(params.pageSize));
    if (params?.practitioner_status) qs.set("practitioner_status", params.practitioner_status);
    const query = qs.toString() ? `?${qs.toString()}` : "";
  ...
  getAnnuitySummary: (
    federationId: string,
    /** `dojo_status` precisa ser o MESMO valor mandado para
     *  listDojoAnnuities, e `practitioner_status` (F6.5, 13/08/2026) o
     *  MESMO valor mandado para listCpfAnnuities — as rotas compartilham
     *  o predicado no backend. Omitidos = default do backend ("active"
     *  pros dois). */
    params?: { year?: string; dojo_status?: AnnuityDojoStatusFilter; practitioner_status?: AnnuityPractitionerStatusFilter }
