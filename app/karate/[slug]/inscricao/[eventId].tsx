...
type Step = "evento" | "categoria" | "tipo" | "cpf" | "convidado" | "confirma" | "pix";
...
const STEP_IDX: Record<Step, number> = { evento: 0, categoria: 1, tipo: 1, cpf: 2, convidado: 2, confirma: 3, pix: 4 };
...
export default function InscricaoScreen() {
  ...
  const doSubmitGuest = async () => {
    // Não-filiado preenche todos os campos (feedback federação): nome, CPF,
    // nascimento, e-mail, telefone, faixa, dojô e professor.
    const obrig: [string, string][] = [
      ["nome completo", guest.name], ["CPF", guest.cpf], ["data de nascimento", guest.birth_date],
      ["e-mail", guest.email], ["telefone", guest.phone], ["faixa", guest.belt],
      ["dojô", guest.dojo], ["professor", guest.professor],
    ];
    const faltando = obrig.filter(([, v]) => !String(v).trim()).map(([k]) => k);
    if (faltando.length) { setErr("generic"); setErrMsg("Preencha: " + faltando.join(", ") + "."); return; }
    setBusy(true); setErr(null);
    try {
      const r = await karatePortalApi.submitInscricao(
    ...
    return (
      <ScrollView style={styles.page} contentContainerStyle={styles.landingWrap}>
        <EventLanding
          ...
          winWidth={winWidth}
          onCta={() => { setErr(null); setStep(isCompetition ? "categoria" : "tipo"); }}
        />
        <AuraFooter />
      </ScrollView>
    );
      ...
      <Card fed={fedName}>
        ...
        <View style={styles.body}>
          {loadingEvent ? (
            ...
            <>
              <Text style={styles.stepLabel}>{isCompetition ? "ETAPA 3 DE 4" : "ETAPA 2 DE 3"} · FILIAÇÃO</Text>
              <Text style={styles.h2}>Você é filiado?</Text>
              <Text style={styles.sub}>Filiados à {fedName} têm os dados preenchidos automaticamente pelo CPF. Não-filiados preenchem o cadastro na próxima etapa.</Text>
              <View style={{ marginTop: 18, gap: 10 }}>
                <TouchableOpacity style={styles.tipoCard} onPress={() => { setErr(null); setStep("cpf"); }} accessibilityRole="button">
                  <Text style={styles.tipoCardTitle}>Sou filiado</Text>
                  <Text style={styles.tipoCardSub}>Localizar meu cadastro pelo CPF, e-mail ou nº FPKT.</Text>
                </TouchableOpacity>
                <TouchableOpacity style={styles.tipoCard} onPress={() => { setErr(null); setStep("convidado"); }} accessibilityRole="button">
                  <Text style={styles.tipoCardTitle}>Não sou filiado</Text>
                  <Text style={styles.tipoCardSub}>Preencher meus dados manualmente para participar.</Text>
                </TouchableOpacity>
              </View>
              {err === "generic" ? <Text style={styles.inlineErr}>{errMsg}</Text> : null}
            </>
            ...
            <>
              ...
              <TextInput style={styles.input} value={guest.belt} onChangeText={(v) => setGuest({ ...guest, belt: v })} placeholder="Ex.: Preta 1º Dan, Marrom…" placeholderTextColor={KarateColors.ink4} />
            ...
            <>
              ...
              <View style={styles.conf}>
                ...
                {pract?.dojo_name ? <ConfRow k="Dojô" v={pract.dojo_name} /> : null}
                {pract?.birth_date ? <ConfRow k="Nascimento" v={pract.birth_date.split("-").reverse().join("/")} /> : null}
                {pract?.email ? <ConfRow k="E-mail" v={pract.email} /> : null}
                {pract?.phone ? <ConfRow k="Telefone" v={pract.phone} /> : null}
          ...
          <View style={styles.foot}>
            {step === "categoria" ? (
              <>
                ...
                <KarateButton
                  label="Continuar"
                  onPress={() => { setErr(null); setStep("tipo"); }}
              ...
              <>
                <KarateButton label="Voltar" variant="secondary" onPress={() => setStep("tipo")} style={{ flex: 1 }} />
                <KarateButton label={busy ? "Localizando…" : "Continuar"} onPress={doLookup} loading={busy} style={{ flex: 2 }} />
              </>
            ) : step === "convidado" ? (
              <>
                <KarateButton label="Voltar" variant="secondary" onPress={() => { setErr(null); setStep("tipo"); }} style={{ flex: 1 }} />
...
const styles = StyleSheet.create({
  ...
  // Gate "É filiado?" — dois cartões de escolha (filiado / não-filiado).
  tipoCard: { padding: 15, borderWidth: 1, borderColor: KarateColors.border, borderRadius: KarateRadius.md, backgroundColor: KarateColors.glass, gap: 3 } as ViewStyle,
  tipoCardTitle: { fontSize: 15, fontWeight: "700", color: KarateColors.ink } as TextStyle,
  tipoCardSub: { fontSize: 12.5, color: KarateColors.ink3, lineHeight: 17 } as TextStyle,
