...
function KPI({ic,iconColor,label,value,delta,deltaUp,large,onPress,sparkData,sparkColor,idx}:{ic:string;iconColor:string;label:string;value:string;delta?:string;deltaUp?:boolean;large?:boolean;onPress?:()=>void;sparkData?:number[];sparkColor?:string;idx?:number}){
...
function OR({name,due,amount,status,category,onPress}:{name:string;due:string;amount:number|null;status:string;category:string;onPress?:()=>void}){
  const[h,sH]=useState(false);const w=Platform.OS==="web";const sc=status==="pending"?Colors.amber:Colors.ink3;const cl=category==="aura_resolve"?"Aura resolve":"Aura facilita";const cc=category==="aura_resolve"?Colors.green:Colors.amber;
...
const s=StyleSheet.create({
  ...
  da:{flexDirection:"row",alignItems:"center",gap:8,backgroundColor:Colors.amberD,borderRadius:10,padding:12,flexWrap:"wrap"},
  daTxt:{fontSize:12,color:Colors.amber,fontWeight:"500",flex:1,minWidth:120},dl:{fontSize:12,color:Colors.violet3,fontWeight:"600"},
