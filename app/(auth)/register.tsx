...
var LOGO_SVG = "https://cdn.jsdelivr.net/gh/CaioAlexanderx/aura-app@main/assets/aura-icon.svg";
var API_BASE = "https://aura-backend-production-f805.up.railway.app/api/v1";
...
var STEPS = ["Sua conta", "Sua empresa"];
...
var webInputProps = isWeb ? { className: "auth-input" } as any : {};
var inputOutline = isWeb ? { outlineWidth: 0 } as any : {};

function saveDetectedRegime(regime: string) {
  try { if (typeof localStorage === "undefined") return; var cfg = JSON.parse(localStorage.getItem("aura_config") || "{}"); cfg.detectedRegime = regime; localStorage.setItem("aura_config", JSON.stringify(cfg)); } catch {}
}

function saveCnpjData(data: any) {
  try {
    if (typeof localStorage === "undefined") return;
    var cfg = JSON.parse(localStorage.getItem("aura_config") || "{}");
    if (data.legal_name) cfg.companyName = data.legal_name;
    if (data.trade_name) cfg.tradeName = data.trade_name;
    if (data.address_street) { var parts = [data.address_street, data.address_number, data.address_district, data.address_city, data.address_state].filter(Boolean); cfg.address = parts.join(", "); }
...
export default function RegisterScreen() {
  var { invite_token, invite_email } = useLocalSearchParams<{ invite_token?: string; invite_email?: string }>();
  var isInviteFlow = !!invite_token;

  var [step, setStep] = useState(0);
  var [nome, setNome] = useState("");
  var [email, setEmail] = useState(invite_email || "");
  var [senha, setSenha] = useState("");
  var [confirmarSenha, setConfirmarSenha] = useState("");
  var [showPass, setShowPass] = useState(false);
  var [cnpj, setCnpj] = useState("");
  var [telefoneEmpresa, setTelefoneEmpresa] = useState("");
  var [telefoneContato, setTelefoneContato] = useState("");
  var [empresa, setEmpresa] = useState("");
  var [codigo, setCodigo] = useState("");
  var [cnpjLoading, setCnpjLoading] = useState(false);
  var [cnpjFound, setCnpjFound] = useState<string | null>(null);
  var [cnpjError, setCnpjError] = useState<string | null>(null);
  var [codeValid, setCodeValid] = useState<boolean | null>(null);
  var [codeChecking, setCodeChecking] = useState(false);
  var { register, isLoading } = useAuthStore();

  async function lookupCNPJ(formatted: string) {
    var nums = formatted.replace(/\D/g, ""); if (nums.length !== 14) return;
    ...
    try {
      ...
      var res = await fetch(API_BASE + "/onboarding/cnpj-lookup", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ cnpj: nums }), signal: controller?.signal });
      ...
      if (res.status === 422) { var err = await res.json().catch(function() { return {}; }); setCnpjError(err.error || "CNPJ com situacao irregular."); return; }
      if (!res.ok) { var err2 = await res.json().catch(function() { return {}; }); setCnpjError(err2.error || "Erro na consulta (" + res.status + ")."); return; }
      var data = await res.json();
      var companyName = data.legal_name || data.trade_name || data.razao_social || data.nome_fantasia || "";
  ...
  function handleCnpjChange(v: string) { var masked = maskCNPJ(v); setCnpj(masked); setCnpjFound(null); setCnpjError(null); if (masked.replace(/\D/g, "").length === 14) lookupCNPJ(masked); }

  async function handleCodeBlur() {
    var code = codigo.trim(); if (!code) { setCodeValid(null); return; }
    setCodeChecking(true);
    try { var r = await authApi.validateCode(code); setCodeValid(r.valid); if (r.valid) toast.success("Codigo valido!"); else toast.error(r.error || "Codigo invalido"); }
    catch { setCodeValid(false); toast.error("Codigo invalido"); }
    finally { setCodeChecking(false); }
  }

  var passLength = senha.length >= 8;
  var passUpper = /[A-Z]/.test(senha);
  var passNumber = /[0-9]/.test(senha);
  var passMatch = senha === confirmarSenha && confirmarSenha.length > 0;
  var passValid = passLength && passUpper && passNumber;
  var step1Valid = nome.length > 0 && email.includes("@") && passValid && passMatch;
  var cnpjValid = cnpj.replace(/\D/g, "").length === 14 && !!cnpjFound && !cnpjError;
  var contatoValid = telefoneContato.replace(/\D/g, "").length >= 10;
  var step2Valid = empresa.length > 0 && contatoValid && cnpjValid;
  ...
  async function handleRegister() {
    ...
    try {
      ...
      setTimeout(function() { router.replace(hasTrialCode ? "/(tabs)/onboarding" : "/(tabs)/checkout"); }, 300);
  ...
  var card = (
    <View style={s.card} {...(isWeb ? { className: "auth-card" } as any : {})}>
      ...
      {isInviteFlow && (
        <View style={s.inviteBanner}><Icon name="users" size={14} color={Colors.violet3} /><Text style={s.inviteBannerText}>Voce foi convidado para entrar numa equipe Aura</Text></View>
      )}
      ...
      {step === 1 && (
        <View>
          <View style={s.field}>
            ...
            {cnpjError && <View style={s.cnpjErrWrap}><Text style={s.cnpjErr}>{cnpjError}</Text><Pressable onPress={function() { if (cnpj.replace(/\D/g, "").length === 14) lookupCNPJ(cnpj); }}><Text style={s.cnpjRetry}>Tentar novamente</Text></Pressable></View>}
